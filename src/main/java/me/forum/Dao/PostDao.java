...
public class PostDao {
	...
	public int CountPost(String user) {
		try {
			return jdbcTemplate.queryForObject("select count(mabaiviet) from baiviet where taikhoan=?", new Object[] {user}, new int[] {Types.CHAR}, Integer.class);
		} catch (Exception e) {
			return 0;
		}
	}
	...
	public List<Post> GetPostsLimit(int start, int limit){
		try {
			return jdbcTemplate.query("select * from baiviet order by ngaytao asc limit ?, ?", new Object[] {start, limit},new int[] {Types.INTEGER, Types.INTEGER}, new PostMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
	...
	public List<Post> GetPostsLimitDesc(int start, int limit){
		try {
			return jdbcTemplate.query("select * from baiviet order by ngaytao desc limit ?, ?", new Object[] {start, limit},new int[] {Types.INTEGER, Types.INTEGER}, new PostMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
	...
	public List<Post> GetPostsUserLimit(String taikhoan, int start, int limit){
		try {
			return jdbcTemplate.query("select * from baiviet where taikhoan=? order by ngaytao desc limit ?, ?", new Object[] {taikhoan, start, limit},new int[] {Types.CHAR, Types.INTEGER, Types.INTEGER}, new PostMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
	...
	public List<Post> GetLikePost(String tieude){
		try {
			return jdbcTemplate.query("select * from baiviet where tieude like ?", new Object[] {"%" + tieude + "%"}, new int[] {Types.NVARCHAR}, new PostMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
	...
	public int UpdatePostByID(long id, String tieude, String noidung, String nhom) {
		return jdbcTemplate.update("update baiviet set tieude = ?, noidung = ?, nhom = ? where mabaiviet = ?", tieude, noidung, nhom, id);
	}
	
	public int AddPost(long id, String tieude, String noidung, String nhom, String taikhoan) {
		return jdbcTemplate.update("insert into baiviet(mabaiviet, tieude, noidung, nhom, taikhoan) value(?, ?, ?, ?, ?)",id , tieude, noidung, nhom, taikhoan);
	}
	
	private class PostMapper implements RowMapper<Post>{
		@Override
		public Post mapRow(ResultSet rs, int rowNum) throws SQLException {
			Post post = new Post(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6));
