...
public class NotificationDao {
	...
	public List<Notification> GetByNguoiGui(String nguoiGui){
		try {
			return jdbcTemplate.query("select * from thongbao where nguoigui = ?", new Object[] {nguoiGui}, new int[] {Types.CHAR}, new NotiMapper());
	...
	public Notification GetById(long id){
		try {
			return jdbcTemplate.queryForObject("select * from thongbao where mathongbao = ?", new Object[] {id}, new int[] {Types.BIGINT}, new NotiMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
	...
	public List<Notification> GetByNguoiNhan(String nguoiNhan){
		try {
			return jdbcTemplate.query("select * from thongbao where nguoinhan = ? order by ngaytao desc", new Object[] {nguoiNhan}, new int[] {Types.CHAR}, new NotiMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
	...
	private class NotiMapper implements RowMapper<Notification> {
		...
		public Notification mapRow(ResultSet rs, int rowNum) throws SQLException {
			return new Notification(rs.getLong(1), rs.getBoolean(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7));
		}
