				...
				<div id="listComments">
					<c:forEach items="${comments}" var="i">
						...
						<div class='content rm'>
							...
							<div class='interactive p-1 m-0'>
								<button type='button' class='like p-1'
									onclick="like(${i.getMabinhluan() },true, '${i.getUser().getTaikhoan()}')">
									<i class='fas fa-heart action <c:if test="${ i.IsLiked(userID.getTaikhoan()) }">fas-liked</c:if>'
										id='pl${i.getMabinhluan() }'></i> <span class='count-like'
										id='p${i.getMabinhluan() }'> <c:if
